...
ggsave(file.path(resultsfolder,"Figure 2 - BMDL_100722.pdf"),height=10,width=8)

# For HW (Supplemental Fig 2) - HW vs. MA
...
p2 <- ggplot(hist.df,aes(x=HW.MA)) + geom_histogram() + 
  scale_x_log10(limits=c(min(hist.df$HW.MA),max(hist.df$HW.MA)),breaks=10^c(-4,-3,-2,-1,0,1,2,3,4),   
                labels=trans_format("log10",math_format(10^.x))) +
  theme_classic() + xlab("HW/MA")

p3 <- ggplot(hist.df,aes(x=MA.BMDL,y=HW.BMDL)) + geom_point() +
  geom_abline() +
  scale_x_log10(limits=c(min(errorbar.df[complete.cases(errorbar.df),][,c("HW.BMDL","BMDS.BMDL")])
                         ,max(errorbar.df[complete.cases(errorbar.df),][,c("HW.BMDL","BMDS.BMDL")])),
                breaks=10^c(-6,-5,-4,-3,-2,-1,0,1,2,3),
                labels=trans_format("log10",math_format(10^.x))) +
  scale_y_log10(limits=c(min(errorbar.df[complete.cases(errorbar.df),][,c("HW.BMDL","BMDS.BMDL")])
                         ,max(errorbar.df[complete.cases(errorbar.df),][,c("HW.BMDL","BMDS.BMDL")])),
                breaks=10^c(-6,-5,-4,-3,-2,-1,0,1,2,3),
