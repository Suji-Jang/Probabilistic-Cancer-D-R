...
total.bmds.df <- fread(file.path(bmdfolder,"BMDS_RSD_092822.csv"))
ma.df <- fread(file.path(bmdfolder,"MA_RSD_092822.csv"))
hw.df <- fread(file.path(bmdfolder,"HW_RSD_092822.csv"))
...
ggsave(file.path(figuresfolder,"Figure 2 - BMDL_100722.pdf"),height=10,width=8)

# For HW (Supplemental Fig) - HW vs. MA
...
p2 <- ggplot(hist.df,aes(x=MA.BMDL,y=HW.BMDL)) + geom_point() +
  ...
  scale_y_log10(limits=c(min(errorbar.df[complete.cases(errorbar.df),][,c("HW.BMDL","BMDS.BMDL")])
                         ,max(errorbar.df[complete.cases(errorbar.df),][,c("HW.BMDL","BMDS.BMDL")])),
                breaks=10^-(6:3),labels=trans_format("log10",math_format(10^.x))) +
  xlab("BMDL [MA]") + ylab("BMDL [HW]") + theme_classic() +
  theme(panel.border = element_rect(colour = "black",fill=NA), 
        plot.tag = element_text(size=20, face="bold"),plot.tag.position = c(0.2, 0.95))
...
hist.scatter <- ggarrange(p2,p3,heights=c(5,5),widths=c(5,5)) # SIZE = 5 * 10
sum.plot <- ggarrange(p1,hist.scatter,heights=c(6,4),widths=c(10,10),ncol=1)
ggsave(file.path(figuresfolder,"Supple Figure - BMDL_100722.pdf"),height=10,width=8)
