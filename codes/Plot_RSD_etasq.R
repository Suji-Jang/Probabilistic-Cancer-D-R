...
pdf(file.path(resultsfolder,"RSD_etasq.pdf"))
for (k in 1:3) {
  if (k==1) {
    risklevel <- 1e-4
    RSD.label <- "RSD04"
  ...
  print(ggpairs(RSD.etasq.df[,c(5,7:11)])+ggtitle(RSD.label))
  print(ggplot(RSD.etasq.df)+geom_histogram(aes(RSD.log10ci90))+ggtitle(RSD.label))
  RSD.etasq.df.long <- pivot_longer(RSD.etasq.df[,c(1,7:11)],cols=2:6)
  print(ggplot(RSD.etasq.df.long)+geom_histogram(aes(value))+facet_wrap(~name))
  ...
  print(ggplot(RSD.etasq.df,aes(x="All Chemicals",fill=var_max))+geom_bar(position="fill")+
          xlab("")+ylab("Proportion of chemicals")+
  ...
  print(ggplot(RSD.etasq.df,aes(x=RSD.log10ci90.bin,fill=var_max))+geom_bar(position="fill")+
    xlab("90% Confidence Width (log10 units)")+ylab("Proportion of chemicals")+
    scale_fill_viridis_d("Largest contribution\nto uncertainty",
                         direction=-1,begin=0.2,drop=FALSE)+ggtitle(RSD.label))
  print(ggplot(RSD.etasq.df,aes(x=RSD.log10ci90.bin,fill=var_max))+geom_bar()+
          xlab("90% Uncertainty Width (log10 units)")+ylab("Number of chemicals")+
          scale_fill_viridis_d("Largest contribution\nto uncertainty",
                               direction=-1,begin=0.2,drop=FALSE)+ggtitle(RSD.label))
}
dev.off()
